...
(function () {
  ...
  const menuItems = [
    { title: 'Pelayanan', href: 'pelayanan.html', x: 0, y: 0, alt: 'Menu Pelayanan' },
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:620px){#home-menu10{padding:34px 0 40px}#home-menu10 .home-menu10-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:11px}#home-menu10 .home-menu10-card{min-height:148px;padding:12px 8px}#home-menu10 .home-menu10-icon{width:78px;height:78px;flex-basis:78px;background-size:390px 156px}#home-menu10 .home-menu10-note{display:none}}
    `;
  ...
  function render(section, spriteBase64) {
    const grid = section.querySelector('.home-menu10-grid');
    if (!grid) return;

    const dataUrl = 'url("data:image/webp;base64,' + spriteBase64 + '")';
    grid.textContent = '';
    ...
    menuItems.forEach(function (item) {
      ...
      icon.style.backgroundImage = dataUrl;
  ...
  function loadSprite(section) {
    Promise.all(SPRITE_PARTS.map(function (url) {
      ...
      .then(function (parts) {
        ...
        if (!base64 || !/^UklG/.test(base64)) throw new Error('Sprite tidak valid');
        render(section, base64);
      })
      .catch(function () {
        const grid = section.querySelector('.home-menu10-grid');
