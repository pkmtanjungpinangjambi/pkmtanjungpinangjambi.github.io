(function(){
  ...
  const INSTAGRAM_URL='https://instagram.com/pkm.tanjungpinang.jambi';
  ...
  function addFacebookLink(){
    ...
    box.innerHTML=`<a href="${FACEBOOK_URL}" target="_blank" rel="noopener noreferrer" class="btn btn-outline" aria-label="Buka Facebook resmi Puskesmas Tanjung Pinang">🔵 Facebook Resmi</a><a href="${INSTAGRAM_URL}" target="_blank" rel="noopener noreferrer" class="btn btn-green" aria-label="Buka Instagram resmi Puskesmas Tanjung Pinang">📷 Instagram Resmi</a>`;
  ...
  fetch('/api/social-feed',{headers:{Accept:'application/json'}})
    ...
    .then(result=>{
      ...
      if(status){status.textContent='Diperbarui otomatis dari Instagram resmi @pkm.tanjungpinang.jambi.';status.classList.remove('error');}
    })
    .catch(error=>{
